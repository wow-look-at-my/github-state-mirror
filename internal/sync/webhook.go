...
func (d *WebhookDispatcher) onWorkflowJob(ctx context.Context, event webhook.Event) outcome {
	...
	// The job also names its RUN, and the repo-wide runs listing is rebuilt
	// from run rows. A job payload carries no run-level status, so this may
	// only establish the run's identity and raise its status floor -- never
	// settle it. Best-effort: the job record above is this handler's
	// disposition.
