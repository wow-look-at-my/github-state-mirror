...
// orgDataQuery is the GraphQL query that fetches all non-archived repos and
// their open PRs for a given organization.
const orgDataQuery = `
query($org: String!, $repoCursor: String, $prCursor: String) {
	...
	for {
		...
		for _, gr := range repos.Nodes {
			...
			for _, gpr := range gr.PullRequests.Nodes {
				pr := convertPR(orgLogin, gr.Name, gpr)
