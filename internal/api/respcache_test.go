...
// The contents, git-commit, install-token, and reveal-layer route tests. The
// fake GitHub and the shared assertions they all use live in
...
func TestCachedContents_QueryStringDistinct(t *testing.T) {
	...
	u.contents = func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json; charset=utf-8")
		fmt.Fprintf(w, `{"type":"file","encoding":"base64","size":1,"name":"f","path":"f","content":%q,"sha":%q}`,
			"ref="+r.URL.Query().Get("ref"), "s-"+r.URL.Query().Get("ref"))
	}
