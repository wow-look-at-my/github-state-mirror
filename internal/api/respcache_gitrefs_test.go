...
func TestCachedGitRef_PushAppliesTipToEverySpelling(t *testing.T) {
	...
	for _, target := range spellings {
		...
		// Byte-identical to the fetched answer but for the sha.
...
// A DELETION states no tip, so the row must go: invalidation where the payload cannot answer.
...
// Shape guards: a partial ref, any query parameter, and a non-default Accept all pass through.
...
// The mirror refetches, unprompted, a tip its own PR rows contradict rather than picking a side.
...
func TestCachedGitRef_LaggingPRBaseCostsOneRefetchNotEvery(t *testing.T) {
	...
	// Upstream keeps answering shaTip: the ref row was right, the PR's base.sha lags.
...
// A merged pull_request/closed also states the base branch's new tip.
