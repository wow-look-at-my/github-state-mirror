...
// absorbedCompare is what one comparison contributes to the cache: the
// rendered document, the commits it carries as git-commit rows, and the base
// tip it was computed against.
...
func absorbCompare(owner, repo string, status int, body []byte) (absorbedCompare, bool) {
	...
	// base_commit is the base side's tip at compute time -- the fact that
	// lets a later read notice the branch has moved on (baseBranchMoved). It
	// stays OFF the rendered document: this route's response shape is pinned
	// by its consumers, and the value is row metadata, not an answer.
	baseTip := ""
