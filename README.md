...
- `GET /api/timeline?since=<id>` — timed traffic events for the Timeline chart: incoming webhook deliveries and outgoing proxied requests, each with its real measured duration, lane, and tooltip fields, plus `max_id` (the next `since` cursor) and `retention_start` (the 24h window floor). Omit `since` (or pass 0) for the full retained window (admin only; in-memory, resets on restart)
