...
Uncached reads are also **debounced**: an eligible passthrough `GET` is held
...
| `PASSTHROUGH_DEBOUNCE` | No | `5s` | How long an uncached (passthrough) read is held so identical concurrent requests share one upstream call. `0` forwards immediately. Deliberately makes uncacheable endpoints slow — the delay is the disincentive. Unparseable, negative, or > 30s fails startup. |
